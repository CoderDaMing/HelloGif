...
public class GifDecoder {
	private String DTAG = "GifDecoder";
	...
	public int getLoopCount() {
		Log.d(DTAG, "getLoopCount - loopCount: "+loopCount);
		return loopCount;
	}
	...
	protected void setPixels() {
		...
		if (lastDispose > 0) {
			...
			if (lastBitmap != null) {
				...
				//Log.d(DTAG, "lastBitmap: "+lastBitmap.getByteCount()/1024);
			}
		...
		try {
			...
			image = null;
			Log.d(DTAG, "OutOfMemoryError: "+e.toString());
		}
	}
	...
	protected void readContents() {
		...
		while (!(done || err())) {
			int code = read();
			Log.d(DTAG, "readContents - code: "+code);
			switch (code) {
			case 0x2C: // dec. 44 image separator
				readBitmap();
				if(readComplete) return;// was !readComplete
				break;
			case 0x21: // dec. 33 Extension Block, 8 bytes, all extension blocks begin with 0x21
				code = read();
				Log.d(DTAG, "readContents - case 0x21, code: "+code);
				switch (code) {
	...
	protected void readBitmap() {
		...
		//Log.d(DTAG, "frames.size: "+frames.size());
		// list
		if (transparency) {
			act[transIndex] = save;
		}
	...
	protected void readNetscapeExt() {
		do {
			...
			if (block[0] == 1) {
				...
				loopCount = (b2 << 8) | b1;
				Log.d(DTAG, "readNetscapeExt - loopCount: "+loopCount);
			}
